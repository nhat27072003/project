...
const OrderList = ({ username }) => {
  ...
  const handleStatus = async (orderID) => {
    try {
      console.log(orderID);
      console.log(orders);
      await axios.put(`http://localhost:8081/order/${orderID}`);
      console.log(orders);
      await getOrders();
      // Không cần gọi lại groupedOrders() vì useEffect sẽ tự động cập nhật khi orders thay đổi
      console.log(orders);
...
//     try{
//       console.log(orderID);
//       const result = await axios.put(`http://localhost:8081/order/${orderID}`);
//       await getOrders();
//       await groupedOrders();
//       console.log(result);
//     }
